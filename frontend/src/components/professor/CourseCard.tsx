...
const CourseCard: React.FC<CourseCardProps> = ({
  ...
  return (
    <Link
      ...
      className="block bg-white rounded-lg border-2 border-gray-200 p-6 hover:border-blue-400 hover:shadow-lg transition-all"
    >
      <div className="flex items-start justify-between mb-4">
        ...
        <div className={`flex-shrink-0 ${getScoreBgColor(statistics.averageScore)} rounded-lg px-4 py-2 text-center`}>
          ...
          <p className="text-xs text-gray-600">Medie</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-3 gap-4 pt-4 border-t border-gray-100">
        <div>
          <p className="text-xs text-gray-500 mb-1">Total evaluări</p>
          <p className="text-lg font-semibold text-gray-900">{statistics.totalEvaluations}</p>
        </div>
        <div>
          <p className="text-xs text-gray-500 mb-1">Completate</p>
          <p className="text-lg font-semibold text-gray-900">{statistics.completedEvaluations}</p>
        </div>
        <div>
          <p className="text-xs text-gray-500 mb-1">Completare</p>
      ...
      <div className="mt-4 pt-4 border-t border-gray-100">
        <span className="text-sm font-medium text-blue-600 inline-flex items-center gap-1 hover:gap-2 transition-all">
          Vezi detalii
          <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
